...
CREATE TABLE tasks (
    ...
    type ENUM('ad', 'video', 'app_install', 'survey', 'share') NOT NULL COMMENT '任务类型',
    reward_amount INT NOT NULL DEFAULT 0 COMMENT '奖励金额（分）',
    requirements JSON COMMENT '任务要求配置',
    user_level ENUM('normal', 'signed', 'vip', 'all') DEFAULT 'all' COMMENT '适用用户等级',
...
CREATE TABLE user_level_config (
    ...
    level_code ENUM('normal', 'signed', 'vip') NOT NULL COMMENT '等级代码',
